...
function updateUI(lastItem) {
    ...
    if (cart.length === 0) {
        ...
        // 増減ボタン操作時などは、カートの最後の商品を表示
        const currentLast = cart[cart.length - 1];
        document.getElementById('target-item-name').textContent = currentLast.kana;
        document.getElementById('target-item-price').textContent = `￥${currentLast.price.toLocaleString()}`;
    }
